...
function trendOption(trends: { date: string; count: number }[], isDark: boolean) {
  const t = chartTheme(isDark)
  const gradStops = isDark
    ? [{ offset: 0, color: 'rgba(127,119,221,0.28)' }, { offset: 1, color: 'rgba(127,119,221,0)' }]
    : [{ offset: 0, color: 'rgba(127,119,221,0.18)' }, { offset: 1, color: 'rgba(127,119,221,0)' }]
  ...
  return {
    ...
    tooltip: {
      ...
      axisPointer: { lineStyle: { color: BRAND.purple, width: 1, type: 'dashed' } },
    },
    xAxis: {
      type: 'category',
    ...
    series: [{
      ...
      areaStyle: {
        color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: gradStops },
      },
...
function donutOption(data: { name: string; value: number; color: string }[], isDark: boolean) {
  const t = chartTheme(isDark)
  return {
    backgroundColor: 'transparent',
    tooltip: {
...
function priorityOption(data: { name: string; value: number; color: string }[], isDark: boolean) {
  const t = chartTheme(isDark)
  return {
    backgroundColor: 'transparent',
  ...
  return (
    <Box>
      ...
      <Progress
        ...
        size={6}
        radius="xl"
        styles={{ bar: { background: barColor }, root: { background: 'var(--mantine-color-default-hover)' } }}
      />
    </Box>
  )
}
...
function MetricRow({
  ...
  return (
    ...
    >
      ...
      <Text size="sm" fw={500} style={valueColor ? { color: valueColor } : undefined}>
        {value}
      </Text>
    </Group>
  )
...
const Dashboard = () => {
      ...
      analytics
        ? Object.entries(analytics.sentiment_analysis || {}).map(([key, value]) => {
            ...
            return {
              ...
              value:    value as number,
              barColor:
                k === 'POSITIVE' ? BRAND.green
                : k === 'NEGATIVE' ? BRAND.red
                : BRAND.blue,
            }
          })
        : [],
  ...
  if (error) {
    return (
      <Container py="lg">
        <Text c="red">Error loading dashboard: {(error as Error).message}</Text>
      </Container>
    )
  }
    ...
    <Container size="xl" py="lg">
      <Stack gap="lg">
        ...
        <Group justify="space-between" align="flex-end">
          <Box>
            <Text fw={500} style={{ fontSize: 22, lineHeight: 1.2 }}>Dashboard</Text>
            <Text size="sm" c="dimmed" mt={2}>Welcome back, {user?.username}</Text>
          </Box>
          <Group gap={8}>
            <Button
              variant="default"
        ...
        <SimpleGrid cols={{ base: 2, sm: 3, md: 5 }} spacing={10}>
          <StatCard label="Total tickets" value={stats.total}      icon="solar:chart-2-linear"        accentColor={BRAND.purple} delta="+12 this week"      deltaUp={true}  />
          <StatCard label="Open"          value={stats.open}       icon="solar:bell-linear"            accentColor={BRAND.red}    delta="+4 since yesterday" deltaUp={false} />
          <StatCard label="In progress"   value={stats.inProgress} icon="solar:hourglass-linear"       accentColor={BRAND.amber}  delta="No change" />
          <StatCard label="Resolved"      value={stats.resolved}   icon="solar:check-circle-linear"   accentColor={BRAND.green}  delta="+18 this week"      deltaUp={true}  />
          <StatCard label="Closed"        value={stats.closed}     icon="solar:archive-check-linear"  accentColor={BRAND.gray}   delta="-2 this week"       deltaUp={false} />
        </SimpleGrid>
        ...
        {isManager && (
          <>
            {analyticsLoading ? (
              <Center h={180}><Loader color={BRAND.purple} /></Center>
            ) : (
              <>
                {/* Row 1: Trend + Donut */}
                <SimpleGrid
                ...
                >
                  ...
                  <Paper radius="md" p="md" style={{ border: '0.5px solid var(--mantine-color-default-border)' }}>
                    ...
                    {trends.length > 0 ? (
                      ...
                      <Center h={180}><Text size="sm" c="dimmed">No trend data</Text></Center>
                    )}
                  </Paper>
                  ...
                  <Paper radius="md" p="md" style={{ border: '0.5px solid var(--mantine-color-default-border)' }}>
                    ...
                    <Group gap="lg" align="center" wrap="nowrap">
                      <Box style={{ position: 'relative', width: 140, height: 140, flexShrink: 0 }}>
                        ...
                        >
                          <Text fw={500} style={{ fontSize: 22, lineHeight: 1 }}>{stats.total}</Text>
                          <Text size="xs" c="dimmed">total</Text>
                        </Box>
                      </Box>
                      <Stack gap={8} style={{ flex: 1, minWidth: 0 }}>
                        {donutData.map((d) => (
                          <Group key={d.name} justify="space-between" gap={6} wrap="nowrap">
                            <Group gap={6} wrap="nowrap">
                              <Box style={{ width: 8, height: 8, borderRadius: '50%', background: d.color, flexShrink: 0 }} />
                              <Text size="xs" c="dimmed">{d.name}</Text>
                            </Group>
                            <Text size="xs" fw={500}>{d.value}</Text>
                          </Group>
                        ))}
                      </Stack>
                    </Group>
                    ...
                    {teamWorkload.length > 0 ? (
                      <Stack gap="sm">
                        ...
                        <Group justify="space-between" mt={4}>
                          <Text size="xs" c="dimmed">{teamWorkload.reduce((s, m) => s + m.total_tickets, 0)} total assigned</Text>
                        ...
                        {[
                          ...
                          return (
                            <Group key={m.name} gap={8} wrap="nowrap">
                              ...
                              <Box style={{ width: 64 }}>
                                <Progress value={(m.tickets / 27) * 100} size={5} radius="xl" styles={{ bar: { background: BRAND.purple }, root: { background: 'var(--mantine-color-default-hover)' } }} />
                              </Box>
          ...
          {recentTickets.length > 0 ? (
            <Box style={{ overflowX: 'auto' }}>
              <Table highlightOnHover>
                <Table.Thead>
                  <Table.Tr>
                    {['ID', 'Title', 'Status', 'Priority', 'Created by', 'Assigned to', ''].map((h) => (
                      <Table.Th
                        key={h}
                        style={{
                          fontSize: 11,
                ...
                <Table.Tbody>
                  {recentTickets.map((ticket: Ticket) => (
                    <Table.Tr key={ticket.id}>
                      <Table.Td style={{ color: 'var(--mantine-color-dimmed)', fontSize: 12 }}>#{ticket.id}</Table.Td>
